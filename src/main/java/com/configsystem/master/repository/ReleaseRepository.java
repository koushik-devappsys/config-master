...
public interface ReleaseRepository extends JpaRepository<Release, Long> {
    ...
    @Query(value = "SELECT * FROM releases WHERE region_id = ?1 AND status = 'RELEASED' ORDER BY global_version DESC LIMIT 1", nativeQuery = true)
    Optional<Release> findLatestReleasedByRegion(Integer regionId);
